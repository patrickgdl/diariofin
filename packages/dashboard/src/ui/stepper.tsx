...
>(({ step, children, className, onStepperItemClick, ...props }, ref) => {
  const { activeStep, steps, orientation, labelOrientation, scrollTracking, status, isClickable } = useStepper()
      ...
      >
        <div className={cn("w-max text-start", isVerticalLabel && !isVertical && "text-center")}>
          ...
          {steps[step].description && (
            <p className="text-xs text-slate-500 dark:text-slate-400">{steps[step].description}</p>
          )}
        </div>
      </div>
        ...
        {!isLastStep && (
          <Separator
            className={cn(
              ...
              isVertical ? "ms-6 flex h-auto min-h-[2rem] w-[1px] border-l-2" : "h-[2px] flex-1",
...
export const StepperFooter = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ children, ...props }, ref) => {
    return (
      <div ref={ref} {...props}>
        {children}
