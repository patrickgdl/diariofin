...
import { getTransactionsByCategory, TransactionsByCategoryQuery } from "~/queries/get-transactions-by-category"
import formatDate from "~/utils/format-date"

import useSupabase from "./useSupabase"

function useTransactionsByCategoryQuery(categoryId: string) {
  const client = useSupabase()

  const { data, ...transactionQuery } = useQuery({
    queryKey: ["transactions", { categoryId }],
    queryFn: async () => {
      return getTransactionsByCategory(client, categoryId!).then((result) => result.data || [])
    },
  ...
  const groupedData = React.useMemo(() => {
    ...
    return sortedItems.reduce((acc, item) => {
      ...
      return acc
    }, {} as Record<string, TransactionsByCategoryQuery[0][]>)
...
export default useTransactionsByCategoryQuery
